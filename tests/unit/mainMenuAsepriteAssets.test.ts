...
describe('MainMenuScene Aseprite runtime assets', () => {
  it('메인 메뉴 배경, 타이틀 입자, 모달/버튼 frame은 Aseprite 자산 우선 렌더 경로를 가진다', () => {
    ...
    expect(mainMenuSource).toContain("const MAIN_MENU_FOCUS_ICON_ID = 'skill_mw_arrow'");
    expect(mainMenuSource).toContain('const MAIN_MENU_FOCUS_ICON_SIZE = 14');
    expect(mainMenuSource).toContain('const MAIN_MENU_FOCUS_ICON_OFFSET_X = 114');
    ...
    expect(mainMenuSource).toContain('MAIN_MENU_EXPECTED_FOCUS_ICON_COUNT = 1');
    ...
    expect(mainMenuSource).toContain('const focusIconResource = getSpriteResourceForSkillIcon(MAIN_MENU_FOCUS_ICON_ID)');
    expect(mainMenuSource).toContain('const isFocusIconQueuedByMenu = MAIN_MENU_BUTTON_ICON_SPECS.some((spec) =>');
    expect(mainMenuSource).toContain('this.load.image(focusIconResource.key, focusIconResource.path)');
    ...
    expect(mainMenuSource).toContain('private menuFocusIcon: Phaser.GameObjects.Image | null = null');
    ...
    expect(mainMenuSource).toContain('this._addMenuFocusIcon(width / 2, menuStartY)');
    ...
    expect(mainMenuSource).toContain("setName('main_menu_focus_icon')");
    expect(mainMenuSource).toContain('setDisplaySize(260, 38)');
    expect(mainMenuSource).toContain('icon.setDisplaySize(20, 20)');
    expect(mainMenuSource).toContain('this.menuFocusIcon.setDisplaySize(MAIN_MENU_FOCUS_ICON_SIZE, MAIN_MENU_FOCUS_ICON_SIZE)');
    ...
    expect(mainMenuSource).toContain('this.menuFocusIcon.texture.setFilter(Phaser.Textures.FilterMode.NEAREST)');
    ...
    expect(mainMenuSource).toContain('private _hasMenuFocusIcon(): boolean');
    expect(mainMenuSource).toContain('return this.menuFocusIcon?.active === true');
    expect(mainMenuSource).toContain('private _addMenuFocusIcon(x: number, y: number): void');
    ...
    expect(mainMenuSource).toContain('return this._hasMenuButtonIcon(i) || this._hasMenuFocusIcon() ? label : (i === this.menuHighlightIndex ? `▶  ${label}` : `   ${label}`)');
    expect(mainMenuSource).toContain('this.menuFocusIcon?.setPosition(activeButton.x - MAIN_MENU_FOCUS_ICON_OFFSET_X, activeButton.y)');
    ...
    expect(mainMenuSource).toContain('menuFocusIcon: {');
    expect(mainMenuSource).toContain('expectedCount: MAIN_MENU_EXPECTED_FOCUS_ICON_COUNT');
    expect(mainMenuSource).toContain('menuLabelLegacyGlyphPresent');
    ...
    expect(mainMenuSource).toContain('missingMenuFocusIconKeys');
