...
describe('QUEST-QA-S1 — 60 퀘스트 정량 정합성', () => {
  it('전체 퀘스트 = 60개 (main 15 + sub 20 + daily 15 + weekly 5 + event 5)', () => {
    expect(ALL_QUEST_SEEDS.length).toBe(60);
  });

  it('group 별 정량: main=15, sub=20, daily=15, weekly=5, event=5', () => {
    expect(QUEST_SEED_GROUPS.main.length).toBe(15);
    expect(QUEST_SEED_GROUPS.sub.length).toBe(20);
