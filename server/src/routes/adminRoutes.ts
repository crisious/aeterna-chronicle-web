...
export async function adminRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  /** 활성 제재 목록 조회 */
  fastify.get('/admin/sanctions', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Querystring: { page?: string; limit?: string } }>, reply: FastifyReply) => {
  ...
  /** 특정 유저 제재 이력 조회 */
  fastify.get('/admin/sanctions/user/:id', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const sanctions = await getUserSanctions(request.params.id);
    return reply.send({ userId: request.params.id, sanctions });
  });

  /** 수동 제재 적용 */
  fastify.post('/admin/sanctions', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Body: { userId: string; level: string; reason: string; evidence?: string } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
  ...
  /** 제재 해제 */
  fastify.delete('/admin/sanctions/:id', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string }; Body: { reason: string } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
  ...
  /** 티켓 목록 (필터) */
  fastify.get('/admin/tickets', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Querystring: Record<string, string> }>, reply: FastifyReply) => {
  ...
  /** 티켓 통계 */
  fastify.get('/admin/tickets/stats', { preHandler: [requireAdmin('admin') as any] }, async (_request, reply: FastifyReply) => {
    const stats = await getTicketStats();
    return reply.send(stats);
  });

  /** 티켓 할당 */
  fastify.post('/admin/tickets/:id/assign', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
  ...
  /** 티켓 답변 */
  fastify.post('/admin/tickets/:id/reply', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string }; Body: { content: string; attachments?: string[] } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
  ...
  /** 티켓 종료 */
  fastify.post('/admin/tickets/:id/close', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string }; Body: { resolution: string } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
  ...
  /** 티켓 우선순위 변경 */
  fastify.patch('/admin/tickets/:id/priority', { preHandler: [requireAdmin('admin') as any] }, async (request: FastifyRequest<{ Params: { id: string }; Body: { priority: string } }>, reply: FastifyReply) => {
    const admin = getAdminUser(request);
