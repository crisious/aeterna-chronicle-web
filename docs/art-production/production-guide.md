...
`DungeonScene` 보스 경고 연출은 `skill_ek_explode.png`를 `dungeon_boss_warning_icon` `30x30` 이미지로 먼저 표시합니다. icon texture가 있으면 경고 label은 `WARNING\n보스 등장!`처럼 glyph 없는 텍스트만 유지하고, texture 누락 시에만 기존 `⚠ WARNING ⚠\n보스 등장!` fallback label을 사용합니다. `dungeonFrameQa=boss` 경로는 보스 경고 상태로 직접 진입해 `bossWarningIcon`, `bossWarningLegacyGlyphPresent`, `missingBossWarningIconKeys`를 기록합니다.
...
`BattleUI` 전투 로그 하이라이트도 Aseprite icon을 먼저 사용합니다. 크리티컬/보스 강공은 `skill_ek_explode.png`, CHAIN/콤보/ECHO/스킬 발동/2인 협공은 `skill_mw_storm.png`, MP 부족은 `skill_mw_passive.png`, 포션 회복은 `ITM-CON-001.png`, 쿨다운/대기/재연결 복구/ATB 모드는 `skill_tg_stop.png`, 자동 전투 모드는 `skill_tg_haste.png`, BGM 재생/누락은 `battle_bgm_playing.png`와 `battle_bgm_missing.png` UI icon, 승리와 3인 협공은 `skill_ek_ultimate.png`, 레벨 업은 `skill_ek_passive.png`, 방어/반사는 `status_shield.png`, 사망/패배는 `status_curse.png`를 `16x16` 이미지로 렌더링합니다. 아이콘이 있을 때 하이라이트 text는 `CRIT 88`, `크리티컬!`, `CHAIN ×2`, `스킬 발동: ...`, `MP 부족 — ...`, `Erien HP +100 회복!`, `에테르 슬래시 쿨다운 중`, `Erien 대기`, `재연결됨 — 전투 재개`, `[AUTO] 자동 전투 ON (×1.5)`, `ATB 모드: WAIT — 메뉴/조준 중 정지`, `BGM: bgm_ancient_field`, `BGM 미존재: bgm_missing`, `협공 발동: 크로노 블레이드`, `3인 협공 발동: 에테르나 파이널`, `방어 태세!`, `반사 → ...`, `승리!`, `레벨 업`처럼 glyph 없는 텍스트만 표시합니다. `BattleScene` 크리티컬 피해/포션 회복/쿨다운/대기/재연결 복구/자동 전투/ATB 모드/BGM 재생/BGM 누락 로그도 `크리티컬!`, `회복`, `쿨다운`, `대기`, `재연결됨 — 전투 재개`, `[AUTO] 자동 전투`, `ATB 모드`, `BGM: <track>`, `BGM 미존재: <track>` 키워드만 전달하고 `💥`, `🧪`, `⏳`, `⏭`, `🔌`, `⚙`, `⏱`, `🎵`, `🔇` glyph는 직접 주입하지 않습니다. icon texture가 없을 때만 기존 `💥`, `🔥`, `🎉`, `🆙`, `⚡`, `✨`, `🌟`, `🔁`, `🏆`, `🛡`, `💀`, `💔`, `💢`, `💧`, `⏳`, `⏭`, `🧪`, `🔌`, `⚙`, `⏱`, `🎵`, `🔇` glyph fallback을 유지합니다. `?debugScene=battle&renderer=canvas&battleLogHighlightIconQa=critical|chain|victory|level|start|echo|skillHit|mana|itemHeal|cooldown|wait|reconnect|autoMode|atbMode|bgmTrack|bgmMissing|telegraph|dualTech|tripleTech|guard|death|defeat`는 `aeternaBattleLogHighlightIconQa`에 icon key, 렌더 수, 표시 크기, fallback 여부, 누락 key, legacy glyph 존재 여부를 기록합니다.
