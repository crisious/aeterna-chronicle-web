...
`BattleScene` 보스 강공 예고 표식은 `skill_ek_explode.png` / `skill_ek_explode_icon` Aseprite skill icon을 먼저 사용합니다. `_showBossTelegraph()`는 `battle_boss_telegraph_icon`을 `30x30` image로 렌더하고, 붉은 tint, 경고음, 전투 로그는 기존 타이밍을 유지합니다. icon texture가 없을 때만 기존 `⚠` text fallback을 사용하므로 정상 렌더 경로에는 경고 glyph가 남지 않습니다.
