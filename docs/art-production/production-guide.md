...
`GameScene` 필드 NPC는 `spriteResourceManifest`의 NPC spritesheet를 먼저 사용하고, 기존 `NPC_SPRITE_MAP` texture가 있으면 legacy texture를 fallback으로 사용합니다. 두 경로가 모두 없을 때는 `assets/generated/ui/placeholders/placeholder.png`를 `placeholder` key로 preload해 `48x64` Aseprite placeholder image를 먼저 렌더하고, placeholder texture까지 없을 때만 기존 `32x48` 절차 사각형 fallback을 생성합니다. `?debugScene=game&renderer=canvas&fieldNpcFallbackQa=1`은 `aeternaGameFieldNpcFallbackQa`에 rendered count, 표시 크기, procedural rectangle 여부, 누락 key를 기록합니다.
...
`DungeonScene` 플레이어는 `characterSpriteManifest`의 full character spritesheet frame을 먼저 사용합니다. spritesheet가 없을 때는 현재 class id에 맞는 `client/public/assets/generated/characters/class_main/battle/char_battle_*.png` Aseprite 전투 썸네일을 `56x84` 이미지로 먼저 렌더하고, 이 썸네일 texture까지 없을 때만 legacy `dungeon_player` side illustration과 절차 사각형 fallback을 순서대로 사용합니다. `?debugScene=dungeon&renderer=canvas&dungeonFrameQa=ready&dungeonPlayerThumbnailFallbackQa=1`은 spritesheet 경로를 의도적으로 건너뛰고 `aeternaDungeonPlayerThumbnailFallbackQa`에 class id, texture key/path, rendered count, 표시 크기, side illustration/rectangle fallback 여부, 누락 key를 기록합니다.

`GameScene` 로컬 플레이어는 `characterSpriteManifest` full spritesheet frame을 먼저 사용합니다. 로컬 class spritesheet가 없거나 `?debugScene=game&renderer=canvas&localPlayerFallbackQa=1`로 spritesheet 경로를 의도적으로 건너뛰면 `char_battle_<classId>.png` Aseprite 전투 썸네일을 physics sprite texture로 먼저 사용하고, 이 썸네일 texture까지 없을 때만 legacy `player_sprite` front illustration/절차 texture fallback을 사용합니다. `aeternaGameLocalPlayerFallbackQa`는 class id, texture key, thumbnail key/path, 표시 크기, legacy fallback 여부, 누락 key를 기록합니다. 원격 플레이어도 같은 full spritesheet 우선 원칙을 따르며, 원격 class spritesheet가 없거나 `remotePlayerFallbackQa=1`이면 전투 썸네일을 `40x60` image로 먼저 렌더하고, 썸네일 texture까지 없을 때만 기존 `40x56` 절차 사각형 fallback을 사용합니다. `aeternaGameRemotePlayerFallbackQa`는 class id, thumbnail key/path, 표시 크기, rectangle fallback 여부, 누락 key를 기록합니다.

`BattleScene` 아군은 `characterSpriteManifest` full spritesheet frame을 먼저 사용하고, class별 `char_battle_<classId>.png` 전투 썸네일을 fallback으로 사용합니다. 미등록 class처럼 두 리소스가 모두 없을 때는 `assets/generated/ui/placeholders/placeholder.png`를 `placeholder` key로 preload해 `48x64` Aseprite placeholder image를 먼저 렌더하고, placeholder texture까지 없을 때만 기존 파란 절차 사각형 fallback을 생성합니다. `?debugScene=battle&renderer=canvas&battleAllyFallbackQa=1&class=missing_class`는 `aeternaBattleAllyFallbackQa`에 expected/rendered count, image key/name, 표시 크기, rectangle fallback 여부, 누락 key를 기록합니다.

`BattleScene`은 몬스터 전용 sprite manifest와 legacy `mon_battle_*` PNG가 모두 없을 때 `assets/generated/monsters/fallback/battle_monster_fallback.png` 또는 `battle_boss_fallback.png`를 먼저 사용합니다. `GameScene`도 필드 몬스터 sprite manifest가 없는 미등록 몬스터에 같은 Aseprite fallback PNG를 먼저 사용하며, `battle_monster_fallback`은 `56x56`, `battle_boss_fallback`은 `72x72` 필드 표시 크기로 렌더합니다. 두 PNG는 `npm run art:battle:monster-fallbacks`로 재생성하며, Aseprite PNG key가 없을 때만 `_mon_prog_*`/필드 절차 사각형과 이모지 fallback을 생성합니다. `?debugScene=game&renderer=canvas&fieldMonsterFallbackQa=1`은 `aeternaGameFieldMonsterFallbackQa`에 expected/rendered image count, legacy emoji 여부, 누락 key를 기록합니다.
...
`MainMenuScene` 타이틀 배경은 `ERB-BG-SKY-DUSK.png`와 `ERB-BG-MID-DUSK.png` Aseprite 환경 배경을 먼저 렌더하고, `ERB-BG-SKY-DUSK` key가 없을 때만 기존 절차형 그라디언트 fallback을 생성합니다. 로그인/회원가입 모달은 `UI-SET-001-DEF.png`, 크레딧 오버레이는 `UI-HUD-005-DEF.png`를 먼저 렌더하며, 메인 메뉴의 게임 시작/설정/크레딧 버튼은 `UI-BTN-006-DEF.png`를 `ui_frame_main_menu_button` key로 preload해 버튼 배경 frame을 먼저 표시합니다. 세 버튼 내부 아이콘은 `skill_mw_arrow.png`, `skill_tg_reverse.png`, `ITM-QST-004.png`를 `20x20` Aseprite image로 먼저 표시합니다. 선택 focus marker도 `skill_mw_arrow.png` / `skill_mw_arrow_icon`을 `main_menu_focus_icon` `14x14` image로 먼저 표시하고, keyboard/pointer focus 이동 시 같은 image 위치만 현재 버튼 왼쪽으로 갱신합니다. texture가 있을 때 메뉴 label은 텍스트만 유지하고, focus icon texture가 없을 때만 기존 `▶` prefix fallback을 사용합니다. 로그인/가입/닫기 및 크레딧 닫기 버튼은 같은 PNG를 `ui_frame_main_menu_modal_button` key로 분리 preload해 모달 action frame으로 사용합니다. 로그인/크레딧 모달 닫기 액션은 `skill_tg_reverse.png` / `skill_tg_reverse_icon`을 `16x16` Aseprite image로 먼저 표시하고, texture가 없을 때만 작은 ASCII `x` fallback을 사용합니다. 로그인 DOM input 2개도 같은 PNG를 `ui_frame_main_menu_modal_input` key로 분리 preload하고 CSS background frame으로 사용합니다. 메뉴 label, keyboard focus index, pointer click, 인증 흐름은 동적 Phaser UI 로직으로 유지하고, modal frame key가 없을 때만 기존 검은 사각형 fallback을 생성합니다. `mainMenuFrameQa=1`은 `menuButtonIcon`, `menuFocusIcon`, `menuLabelLegacyGlyphPresent`, `missingMenuButtonIconKeys`, `missingMenuFocusIconKeys`, `modalCloseIcon`, frame/input 상태를 함께 기록합니다.
...
`WorldScene` 액션 버튼 내부 아이콘은 Aseprite skill icon을 먼저 사용합니다. 이전 시대 버튼은 `skill_tg_reverse.png`, 다음 시대 버튼은 `skill_tg_haste.png`, 마을 복귀 버튼도 `skill_tg_reverse.png`, 시간 이동 버튼은 `skill_mw_arrow.png`를 `18x18` 이미지로 렌더링하고, 아이콘이 있을 때 label은 단축키/텍스트만 표시합니다. `WorldActionButtonOptions.fallbackLabel`은 icon texture가 없을 때만 기존 `◀`, `▶`, `←` 기호 포함 label을 사용하게 분리되어 있습니다. 마을 복귀와 이전 시대가 같은 reverse icon을 공유하므로 `WorldScene.preload()`는 동일 texture key를 한 번만 큐잉합니다. `worldFrameQa=1` 경로는 `actionButtonIcon` 렌더 상태와 함께 `actionButtonText.labels`, `actionButtonText.legacyGlyphPresent`를 기록해 Aseprite icon 사용 시 버튼 label에 방향 glyph가 남지 않았는지 검증합니다.
...
`LobbyScene` 마을 제목은 `zone_aether_plains.png` / `zone_aether_plains` 월드맵 아이콘을 `20x20` 이미지로 먼저 표시합니다. 아이콘 texture가 있으면 제목 text는 `아에테리아 마을`만 유지하고 `☆` 장식 문자는 사용하지 않으며, texture가 없을 때만 기존 `☆ 아에테리아 마을 ☆` fallback label을 사용합니다. `?debugScene=lobby&renderer=canvas&lobbyTitleIconQa=1` 경로는 `aeternaLobbyTitleIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, legacy glyph 존재 여부, 누락 key 상태를 기록합니다.

`LobbyScene` NPC action 알림은 기존 `🛒`/`🔨`/`📜`/`⚔`/`📖` prefix 대신 상점 `ITM-CON-001.png`, 강화 `ITM-MAT-001.png`, 퀘스트 `ITM-QST-004.png`, 파티 `skill_ek_slash.png`, 스토리 `ITM-QST-001.png` Aseprite icon을 재사용합니다. 런타임 표시는 `lobby_notification_icon` `18x18` image와 텍스트-only label 조합이며, texture가 없을 때만 기존 텍스트 알림 fallback으로 남깁니다. `?debugScene=lobby&renderer=canvas&lobbyNotificationIconQa=shop|enhance|quest|party|story` 경로는 `aeternaLobbyNotificationIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, legacy glyph 존재 여부, 누락 key 상태를 기록합니다.

`LoadingScene` tip 라인은 `skill_mw_bolt.png` / `skill_mw_bolt_icon` Aseprite skill icon을 `18x18` 이미지로 먼저 표시합니다. tip 본문은 아이콘이 있을 때 문장만 유지하고, icon texture가 없을 때만 기존 `💡` glyph prefix fallback을 사용합니다. `?debugScene=loading&renderer=canvas`는 `aeternaLoadingFrameQa.tipIcon`에 icon key, 표시 크기, fallback 여부, legacy glyph 존재 여부, 누락 key를 기록합니다.
...
`DungeonScene` 클리어 보상 패널의 EXP/Gold 행은 `skill_ek_passive.png`와 `ITM-MAT-002.png`를 각각 `18x18` 이미지로 먼저 표시합니다. 보상 값 텍스트와 로비 복귀 입력은 기존 Phaser UI 로직으로 유지하고, `dungeonFrameQa=clear` 경로는 `rewardIcon`, `missingRewardIconKeys`, `rewardLabelLegacyGlyphPresent`에 보상 아이콘 렌더 상태를 기록합니다.

`DungeonScene` 몬스터 preview texture가 없거나 `dungeonMonsterFallbackQa=1`로 preview를 우회할 때는 `battle_monster_fallback.png`와 `battle_boss_fallback.png`를 generic Aseprite monster fallback image로 먼저 표시합니다. 웨이브 수, 몬스터명 label, 전투 시작 입력은 기존 동적 Phaser 로직으로 유지하고, `aeternaDungeonMonsterFallbackQa`는 fallback image 수, texture key, legacy emoji fallback 여부, 누락 key를 기록합니다.

`DungeonScene` 퇴장/클리어 복귀 버튼은 `skill_tg_reverse.png`를 `dungeon_exit_action_icon` `16x16`, `dungeon_return_action_icon` `18x18` 이미지로 먼저 표시합니다. icon texture가 있으면 label은 `퇴장 (ESC)`와 `로비로 복귀`처럼 텍스트만 유지하고, texture 누락 시에만 기존 `← 퇴장 (ESC)`와 `[ 로비로 복귀 ]` fallback label을 사용합니다. `dungeonFrameQa=ready|clear` 경로는 `exitActionIcon`, `returnActionIcon`, `missingExitActionIconKeys`, `missingReturnActionIconKeys`, `exitActionLegacyGlyphPresent`, `returnActionLegacyGlyphPresent`에 던전 이동 액션 아이콘 렌더 상태를 기록합니다.

`DungeonScene` 보스 경고 연출은 `skill_ek_explode.png`를 `dungeon_boss_warning_icon` `30x30` 이미지로 먼저 표시합니다. 보스 preview 이름 라벨도 같은 Aseprite icon을 `dungeon_boss_name_icon_*` `18x18` 이미지로 먼저 표시하고, icon texture가 있으면 이름 text는 `기억 흡수자`처럼 glyph 없는 텍스트만 유지합니다. texture 누락 시에만 기존 `⚠ WARNING ⚠\n보스 등장!` 경고 label과 `★ <보스명> ★` 이름 label fallback을 사용합니다. `dungeonFrameQa=boss` 경로는 보스 경고와 보스 preview 상태로 직접 진입해 `bossWarningIcon`, `bossNameIcon`, `bossWarningLegacyGlyphPresent`, `bossNameLegacyGlyphPresent`, `missingBossWarningIconKeys`, `missingBossNameIconKeys`를 기록합니다.
...
`BattleUI` 전투 로그 하이라이트도 Aseprite icon을 먼저 사용합니다. 크리티컬/보스 강공은 `skill_ek_explode.png`, CHAIN/콤보/ECHO/스킬 발동/2인 협공은 `skill_mw_storm.png`, MP 부족은 `skill_mw_passive.png`, 포션 회복은 `ITM-CON-001.png`, 쿨다운/대기/재연결 복구/ATB 모드와 도주 실패/차단/불가는 `skill_tg_stop.png`, 자동 전투 모드는 `skill_tg_haste.png`, BGM 재생/누락은 `battle_bgm_playing.png`와 `battle_bgm_missing.png` UI icon, 승리와 3인 협공은 `skill_ek_ultimate.png`, 레벨 업은 `skill_ek_passive.png`, 도주 성공/비상 도주는 `skill_vw_warp.png`, 방어/반사는 `status_shield.png`, 사망/패배는 `status_curse.png`를 `16x16` 이미지로 렌더링합니다. 아이콘이 있을 때 하이라이트 text는 `CRIT 88`, `크리티컬!`, `CHAIN ×2`, `스킬 발동: ...`, `MP 부족 — ...`, `Erien HP +100 회복!`, `에테르 슬래시 쿨다운 중`, `Erien 대기`, `재연결됨 — 전투 재개`, `[AUTO] 자동 전투 ON (×1.5)`, `ATB 모드: WAIT — 메뉴/조준 중 정지`, `BGM: bgm_ancient_field`, `BGM 미존재: bgm_missing`, `도주 성공!`, `도주 실패!`, `도주 차단!`, `도주 불가!`, `비상 도주!`, `협공 발동: 크로노 블레이드`, `3인 협공 발동: 에테르나 파이널`, `방어 태세!`, `반사 → ...`, `승리!`, `레벨 업`처럼 glyph 없는 텍스트만 표시합니다. `BattleScene` 크리티컬 피해/포션 회복/쿨다운/대기/재연결 복구/자동 전투/ATB 모드/BGM 재생/BGM 누락 로그도 `크리티컬!`, `회복`, `쿨다운`, `대기`, `재연결됨 — 전투 재개`, `[AUTO] 자동 전투`, `ATB 모드`, `BGM: <track>`, `BGM 미존재: <track>` 키워드만 전달하고 `💥`, `🧪`, `⏳`, `⏭`, `🔌`, `⚙`, `⏱`, `🎵`, `🔇` glyph는 직접 주입하지 않습니다. 도주 로그는 `composeEscapeLog()` SSOT가 기존 `🏃`/`❌` 원문을 유지하고, BattleUI 표시 계층에서만 Aseprite icon을 붙인 뒤 `🏃`, `❌`, `🚧`, `🔒`, `🆘` glyph를 제거합니다. icon texture가 없을 때만 기존 `💥`, `🔥`, `🎉`, `🆙`, `⚡`, `✨`, `🌟`, `🔁`, `🏆`, `🛡`, `💀`, `💔`, `💢`, `💧`, `⏳`, `⏭`, `🧪`, `🔌`, `⚙`, `⏱`, `🎵`, `🔇`, `🏃`, `❌`, `🚧`, `🔒`, `🆘` glyph fallback을 유지합니다. `?debugScene=battle&renderer=canvas&battleLogHighlightIconQa=critical|chain|victory|level|start|echo|skillHit|mana|itemHeal|cooldown|wait|reconnect|autoMode|atbMode|bgmTrack|bgmMissing|escapeSuccess|escapeFail|escapeBlocked|escapeForbidden|escapeCritical|telegraph|dualTech|tripleTech|guard|death|defeat`는 `aeternaBattleLogHighlightIconQa`에 icon key, 렌더 수, 표시 크기, fallback 여부, 누락 key, legacy glyph 존재 여부와 `escapeLegacyGlyphPresent`를 기록합니다.
...
`BattleScene` 보스 협공 저항/면역 라벨은 `status_shield.png` Aseprite status icon을 먼저 사용합니다. `_updateBossImmuneLabel()`과 `_updateBossResistLabel()`은 `battle_boss_resist_icon_<unitId>`를 `16x16` image로 캐시해 보스 sprite 위에 붙이고, icon texture가 있으면 label text는 `협공 면역` 또는 `Dual +N% / Triple +N%`처럼 glyph 없는 텍스트만 유지합니다. `status_shield_icon` texture가 없을 때만 기존 `🛡 협공 면역` / `🛡 Dual +N%` fallback 문자열을 사용합니다.
...
`BattleScene` 콤보 보너스 팝업도 `skill_mw_storm.png` Aseprite skill icon을 먼저 사용합니다. `_spawnComboText()`는 `battle_combo_popup_icon`을 `18x18` image로 렌더하고, 아이콘이 있을 때 popup text는 `<combo name> +<bonus>%`만 표시합니다. `skill_mw_storm_icon` texture가 없을 때만 기존 `⚡ <combo name> +<bonus>%` 문자열 fallback을 사용합니다. `?debugScene=battle&renderer=canvas&battleComboPopupIconQa=1`는 `aeternaBattleComboPopupIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, 누락 key, legacy glyph 존재 여부를 기록합니다.
...
`BattleScene` 속성 데미지 태그도 Aseprite skill icon을 먼저 사용합니다. 화염은 `skill_ek_explode.png`, 얼음은 `skill_mw_storm.png`, 번개는 `skill_mw_bolt.png`, 그림자는 `skill_mw_ultimate.png`, 신성은 `skill_mw_heal.png`를 각각 `battle_element_tag_icon` `16x16` image로 렌더하고, icon texture가 있으면 태그 label은 `화염`, `얼음`, `번개`, `그림자`, `신성` 텍스트만 표시합니다. 기존 `🔥 화염`, `❄ 얼음`, `⚡ 번개`, `🌑 그림자`, `✨ 신성` 문자열은 texture 누락 fallback에서만 유지합니다. `?debugScene=battle&renderer=canvas&battleElementTagIconQa=fire|ice|lightning|shadow|holy`는 `aeternaBattleElementTagIconQa`에 icon key/path, 표시 크기, fallback 여부, 누락 key, legacy glyph 존재 여부를 기록합니다.

`BattleScene` 보스 강공 예고 표식은 `skill_ek_explode.png` / `skill_ek_explode_icon` Aseprite skill icon을 먼저 사용합니다. `_showBossTelegraph()`는 `battle_boss_telegraph_icon`을 `30x30` image로 렌더하고, 붉은 tint, 경고음, 전투 로그는 기존 타이밍을 유지합니다. icon texture가 없을 때만 기존 `⚠` text fallback을 사용하므로 정상 렌더 경로에는 경고 glyph가 남지 않습니다. `?debugScene=battle&renderer=canvas&battleBossTelegraphIconQa=1`는 첫 enemy를 deterministic boss로 지정하고 `aeternaBattleBossTelegraphIconQa`에 icon key, 렌더 수, 표시 크기, fallback 여부, 누락 key, legacy glyph 존재 여부를 기록합니다.

`EffectManager`의 풀링 데미지/협공 텍스트도 Aseprite skill icon을 먼저 사용합니다. `preloadEffectTextIconResources()`는 `skill_ek_explode.png`와 `skill_mw_storm.png`를 전투 preload 큐에 연결하고, `DamageTextItem`은 재사용 가능한 `Image`를 함께 보유합니다. `spawnDamageText(..., isCritical=true)`는 치명타 텍스트를 숫자만 표시하고 `skill_ek_explode_icon`을 `20x20` image로 붙이며, `spawnDualTechEffect()`는 협공명 텍스트 앞에 `skill_mw_storm_icon`을 붙입니다. 아이콘 위치/alpha/depth는 기존 텍스트 풀 업데이트 루프와 동기화해 추가 GC 없이 유지합니다.
...
`BattleScene` 타겟 선택 커서도 Aseprite `skill_mw_arrow.png` / `skill_mw_arrow_icon`을 재사용합니다. `_drawTargetCursor()`는 선택 대상 머리 위에 `battle_target_cursor_icon` `24x24` 이미지를 90도 회전해 먼저 표시하고, 대상 인덱스 변경, 예상 데미지/KILL preview, 클릭 타겟팅은 기존 로직을 유지합니다. texture가 없을 때만 기존 `Graphics.strokeTriangle()` 절차 삼각형 fallback을 사용합니다.

`BattleScene` 하단 상태 패널의 HP 위험 표시는 Aseprite `status_bleed.png` / `status_bleed_icon`을 재사용합니다. `_createStatusPanel()`은 `battle_hp_critical_icon_<unitId>`를 `12x12` image로 먼저 만들고, `_updateStatusPanel()`은 HP 25% 미만일 때 해당 image를 표시합니다. icon texture가 없거나 image가 비활성화된 경우에만 기존 `⚠ HP` 텍스트 fallback을 사용하므로 정상 경로에는 경고 glyph가 남지 않습니다.
...
`BattleScene` 마법/아이템 서브메뉴도 row 앞 아이콘을 Aseprite PNG로 먼저 표시합니다. 마법 서브메뉴는 각 `SkillSlot.icon`을 `getSpriteResourceForSkillIcon()`으로 해결해 `battle_magic_submenu_icon_<skillId>` 이미지를 만들고, 쿨다운 상태는 `skill_tg_stop.png`를 `battle_magic_submenu_cooldown_icon_<skillId>` `14x14` 이미지로 추가 표시합니다. 아이템 서브메뉴의 포션 row는 `ITM-CON-001.png`를 `battle_item_submenu_icon_ITM-CON-001` 이미지로 렌더링합니다. 스킬명, MP 비용, 쿨다운 초, 포션 회복 로그, pointer/keyboard 선택은 기존 동적 Phaser UI 로직으로 유지하고, normal label/log에는 `⏳`와 `🧪` glyph를 넣지 않습니다. icon texture가 없을 때만 기존 이모지 포함 item label fallback을 사용합니다. MP 부족 로그는 `BattleUI` log highlight에서 `skill_mw_passive.png`를 `16x16` Aseprite image로 먼저 표시하고, 포션 회복 로그는 `ITM-CON-001.png` item icon을 `16x16` Aseprite image로 먼저 표시합니다. `💧`와 `🧪` glyph는 QA/fallback source message에만 유지합니다.
...
`GameScene` 에러 화면 타이틀은 `skill_ek_explode.png` / `skill_ek_explode_icon` Aseprite skill icon을 `game_scene_error_title_icon` `22x22` image로 먼저 표시하고, title text는 `존 로딩 실패`만 유지합니다. icon texture가 없을 때만 기존 `⚠️ 존 로딩 실패` 문자열 fallback을 사용합니다. `?debugScene=game&renderer=canvas&gameErrorIconQa=1`는 `aeternaGameErrorIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, 누락 key를 기록합니다.

`GameScene` 상단 연결 상태 표시는 Aseprite skill icon을 먼저 사용합니다. 온라인 상태는 `skill_mw_arrow.png` / `skill_mw_arrow_icon`, 오프라인/연결 대기 계열은 `skill_tg_stop.png` / `skill_tg_stop_icon`, 연결 실패는 `skill_ek_explode.png` / `skill_ek_explode_icon`을 `game_scene_connection_status_icon` `14x14` image로 표시하고, label은 `온라인`, `오프라인`, `연결 실패`처럼 glyph 없는 텍스트만 유지합니다. icon texture가 없을 때만 기존 `●`/`○`/`✕` fallback label을 사용합니다. `?debugScene=game&renderer=canvas&gameConnectionIconQa=connected|offline|error`는 `aeternaGameConnectionIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, legacy glyph, missing key 상태를 기록합니다.

`LobbyScene` 하단 연결 상태 표시도 같은 Aseprite skill icon 계약을 사용합니다. 온라인 상태는 `skill_mw_arrow.png` / `skill_mw_arrow_icon`, 오프라인/연결 대기 계열은 `skill_tg_stop.png` / `skill_tg_stop_icon`, 연결 실패는 `skill_ek_explode.png` / `skill_ek_explode_icon`을 `lobby_connection_status_icon` `14x14` image로 표시하고, label은 `온라인`, `오프라인`, `연결 실패`처럼 glyph 없는 텍스트만 유지합니다. icon texture가 없을 때만 기존 `●`/`○`/`✕` fallback label을 사용합니다. `?debugScene=lobby&renderer=canvas&lobbyConnectionIconQa=connected|offline|error`는 `aeternaLobbyConnectionIconQa`에 icon key/path, 렌더 수, 표시 크기, fallback 여부, legacy glyph, missing key 상태를 기록합니다.
...
`SettingsScene` 섹션 제목은 기존 `⚙`, `🔊`, `🌐`, `♿`, `⌨` 텍스트 prefix 대신 `settings_title.png`, `settings_sound.png`, `settings_language.png`, `settings_accessibility.png`, `settings_keybind.png` 32x32 Aseprite UI icon을 먼저 사용합니다. title은 `22x22`, 일반 섹션은 `18x18` 이미지로 표시하고, 제목 텍스트는 `설정`, `사운드`, `언어`, `접근성`, `키바인드`만 유지합니다. texture가 없을 때만 legacy glyph fallback을 사용하며, `?debugScene=settings&renderer=canvas&settingsFrameQa=1`는 `aeternaSettingsFrameQa.settingsSectionIcon`, `settingsSectionLabelLegacyGlyphPresent`, `missingSectionIconKeys`에 expected/rendered count, texture key, 표시 크기, fallback id, 누락 key를 기록합니다.
...
독립 `ChatUI`는 `UI-HUD-008-DEF.png`를 `ui_frame_chat_panel` key로, `UI-BTN-006-DEF.png`를 `ui_frame_chat_input`, `ui_frame_chat_tab_button`, `ui_frame_chat_emoji_button` key로 preload해 채팅 panel, 입력창, 채널 탭, 이모지 button frame을 먼저 표시합니다. 이모지 button 내부 아이콘은 `status_charm.png` / `status_charm_icon`을 `16x16` image로 먼저 렌더하고, texture가 없을 때만 기존 `😀` glyph fallback을 사용합니다. 시스템 메시지 prefix는 `chat_system.png` / `ui_icon_chat_system`을 `14x14` image로 먼저 표시하고, texture가 없을 때만 기존 `⚙️` glyph fallback을 사용합니다. `?debugScene=chat&renderer=canvas&chatFrameQa=1`는 `aeternaChatFrameQa.emojiButtonIcon`, `systemMessageIcon`, `systemMessageLegacyGlyphPresent`, `missingSystemMessageIconKeys`에 texture key, 표시 크기, fallback/missing 상태를 기록합니다. 채널 전환 tint/alpha, 메시지 row, 미읽음 카운트, 이모지 선택, 입력 버퍼, 소켓 이벤트 처리는 기존 동적 Phaser UI 로직으로 유지합니다.
...
`FeedbackForm` 유형 선택 버튼은 Aseprite status icon 리소스를 재사용합니다. 버그/기능/밸런스/UX/기타는 각각 `status_poison_icon`, `status_haste_icon`, `status_shield_icon`, `status_charm_icon`, `status_stun_icon`을 `18x18` 이미지로 표시하고, texture가 없을 때만 기존 이모지 포함 label로 fallback합니다. 키보드 focus marker는 `skill_mw_arrow.png` / `skill_mw_arrow_icon`을 `feedback_form_type_focus_icon` `14x14` 이미지로 하나만 만들고 현재 유형 버튼 왼쪽으로 이동시킵니다. texture가 있을 때 유형 label은 `버그`, `기능`, `밸런스`, `UX`, `기타` 텍스트만 유지하고, texture가 없을 때만 기존 `▶` prefix fallback을 사용합니다. `?debugScene=feedback&renderer=canvas`는 `aeternaFeedbackFrameQa.typeIcon`, `typeFocusIcon`, `missingTypeFocusIconKeys`에 expected/rendered count, texture key 목록, label legacy glyph 여부, fallback id, 누락 key를 기록합니다.
...
`CharacterSelectScene` 캐릭터 생성 card와 기존 캐릭터 row avatar는 Aseprite character battle thumbnail을 먼저 사용합니다. class id별 `char_battle_*.png`를 preload하고, 생성 card에서는 `54x81` 이미지 `character_select_class_card_avatar_<classId>`, 기존 캐릭터 목록에서는 `28x42` 이미지 `character_select_existing_avatar_<classId>`로 표시합니다. 생성 card는 thumbnail texture가 없을 때만 기존 front illustration으로 후퇴하고, 해당 texture도 없을 때만 색 원형 fallback을 사용합니다. 기존 캐릭터 row는 texture가 없을 때만 기존 색 원형 fallback을 사용합니다. `?debugScene=characterSelect&renderer=canvas&characterSelectFrameQa=1`는 `aeternaCharacterSelectFrameQa.classCardAvatar`에 6개 class card avatar의 expected/rendered count, texture key, 표시 크기, fallback id, 누락 key를 기록합니다. `characterSelectExistingQa=1`를 함께 사용하면 `existingCharacterAvatar`에도 기존 row avatar 상태를 기록합니다.

`CharacterSelectScene` 로그아웃 버튼은 기존 `←` 텍스트 glyph 대신 `skill_tg_reverse.png` / `skill_tg_reverse_icon` Aseprite skill icon을 먼저 사용합니다. 아이콘은 좌상단에 `16x16` 이미지 `character_select_logout_icon`으로 표시하고, label은 `로그아웃` 텍스트만 유지합니다. texture가 없을 때만 기존 `← 로그아웃` fallback을 사용하며, `?debugScene=characterSelect&renderer=canvas&characterSelectFrameQa=1`는 `aeternaCharacterSelectFrameQa.logoutIcon`과 `missingLogoutIconKeys`에 렌더 상태, 표시 크기, legacy glyph 여부를 기록합니다.

`SkillTreeUI` 스킬 노드는 Aseprite skill icon 리소스를 먼저 사용합니다. `LobbyScene.preload()`가 `preloadSkillTreeIconResources()`로 6개 클래스의 스킬 트리 후보 icon을 로드하고, `SkillTreeUI`는 각 skill의 `icon` 또는 tier fallback을 `spriteResourceManifest`의 `skillIcon`으로 해석해 `48x48` 이미지 `skill_tree_node_icon_<skillId>`로 표시합니다. 전직 일러스트는 `client/public/assets/generated/characters/class_advanced/char_illust_<class>_adv<n>_front.png`를 `char_<class>_adv<n>` texture key로 preload하고, 스킬 트리 상단에 `52x68` 이미지 `skill_tree_advanced_illustration_<class>_<n>` 3장으로 표시합니다. 제목 아이콘은 현재 클래스의 1티어 skill icon을 `20x20` 이미지 `skill_tree_title_icon`으로 표시하고, 기존 `🌳` glyph는 normal render path에서 제거합니다. 리셋 action button 내부 아이콘은 `skill_tg_reverse.png` / `skill_tg_reverse_icon`을 `18x18` image로 먼저 렌더하고, texture가 있을 때 label은 `스킬 리셋`, `정말 리셋? (다시 선택)`, `리셋 실패`처럼 텍스트만 표시합니다. 메인/상세 닫기 action button도 같은 `skill_tg_reverse_icon`을 `16x16` 이미지 `skill_tree_close_action_icon`, `skill_tree_detail_close_action_icon`으로 먼저 표시하고, texture가 없을 때만 작은 ASCII `x` fallback을 사용합니다. 상세 패널의 분기/잠김 라인은 `skill_mw_storm_icon`, `skill_tg_stop_icon`을 `14x14` 이미지 `skill_tree_branch_detail_icon`, `skill_tree_locked_detail_icon`으로 먼저 표시하고, 기존 `⚔`/`✗` glyph는 normal render path에서 제거합니다. texture가 없을 때만 안전 fallback marker/label을 사용하며, `?debugScene=lobby&renderer=canvas&skillTreeQa=1`는 `aeternaSkillTreeFrameQa.advancedIllustration`, `titleIcon`, `skillNodeIcon`, `resetActionIcon`, `closeActionIcon`, `detailLineIcon`에 expected/rendered count, texture key, 표시 크기, fallback state, 누락 key를 기록합니다.
...
`ErrorBoundary` DOM 오버레이는 오류 제목에 `status_curse.png`, 재연결 배지에 `skill_tg_stop.png` Aseprite icon을 직접 `<img>`로 먼저 표시합니다. 정상 이미지 로드 경로에서는 `예기치 않은 오류`, `재연결 중...` 텍스트만 표시하고 `⚠`, `⚡` glyph는 이미지 로드 실패 fallback span에서만 채웁니다. 자동 QA는 `aeternaErrorBoundaryIconQa`와 `aeternaReconnectIconQa`에 icon id/path, 렌더 수, fallback, legacy glyph 상태를 기록합니다.

`ComboUI`는 `UI-BTN-005-DEF.png`를 `ui_frame_combo_chain_gauge` key로 preload해 우측 상단 chain gauge track frame을 먼저 표시합니다. 콤보 달성 라벨은 `skill_mw_storm.png`를 `skill_mw_storm_icon` texture로 preload해 `28x28` 이미지로 먼저 표시하고, texture가 있으면 label text는 `전격 강타!`처럼 glyph 없는 텍스트만 사용합니다. texture가 없을 때만 기존 `🔥 전격 강타!` fallback을 사용합니다. 콤보 힌트 행의 콤보명/다음 스킬 구분자는 `skill_mw_arrow.png` / `skill_mw_arrow_icon`을 `14x14` Aseprite image로 먼저 표시하고, texture가 없을 때만 기존 `→` fallback을 사용합니다. hit counter, multiplier, gauge fill 색상/감소, hint row text, screen shake setting은 기존 동적 Phaser UI 로직으로 유지합니다. `?debugScene=combo&renderer=canvas&comboFrameQa=1`는 `comboAchievedIcon`, `comboTextLegacyGlyphPresent`, `missingComboAchievedIconKeys`, `hintSeparatorIcon`, `hintTextLegacyArrowPresent`, `missingHintSeparatorIconKeys`에 콤보 달성 및 힌트 구분자 아이콘 렌더 상태를 기록합니다.
...
`GameScene`, `BattleScene`, `DungeonScene`은 manifest의 `textureKey`와 `imagePath`를 사용해 `load.spritesheet(..., { frameWidth: 64, frameHeight: 64 })`로 로드하고, 생성 직후 `setFrame(0)`을 적용합니다. `GameScene`은 로컬 플레이어용 선택 class 외에도 `CHARACTER_SPRITE_MANIFEST` 전체를 preload해 `world:playerJoined.characterClass`가 포함된 원격 플레이어를 같은 Aseprite sheet frame `0`으로 표시합니다. 로컬/원격 GameScene 플레이어와 DungeonScene 플레이어는 manifest sheet가 없을 때 class id별 `char_battle_<classId>.png` 전투 썸네일을 먼저 사용하고, legacy 일러스트/절차 사각형은 안전 fallback으로만 유지합니다. 이 처리가 없으면 `1920x320` sheet 전체가 한 스프라이트처럼 렌더링될 수 있습니다.

`world:playerJoined.characterClass`는 하위 호환을 위해 optional입니다. 값이 없거나 해당 texture가 로드되지 않았을 때만 원격 플레이어 thumbnail/rectangle fallback으로 내려갑니다. `debugScene=game` 기반 그래픽 QA에서는 `offlineQa`가 zone API, socket setup, HUD active quest load를 건너뛰어 백엔드 상태와 무관하게 스프라이트 로드/렌더링만 검증합니다.
...
#### Phase 217: Cutscene class portrait

`CutsceneScene`은 내레이터 포트레이트뿐 아니라 기본 클래스 6종 `char_illust_*_front.png`를 `CUTSCENE_PORTRAIT_TEXTURES`로 preload합니다. `PORTRAIT_MAP`은 에테르 기사, 기억술사, 그림자 직조사, 기억 파괴자, 시간 수호자, 공허 방랑자와 기존 시나리오 역할명을 Aseprite 포트레이트 texture key로 정규화합니다. 직접 지정된 legacy `portrait_*` key는 `CUTSCENE_LEGACY_PORTRAIT_KEY_MAP`을 통해 사용 가능한 Aseprite 포트레이트로 변환하고, texture가 없을 때만 무표시 portrait fallback으로 남깁니다.

`?debugScene=cutscene&renderer=canvas&cutsceneFrameQa=1&cutsceneSpeaker=ether_knight` 경로는 클래스 화자 대화를 시작하고, `aeternaCutsceneFrameQa.portrait`에 `preloadedTextureCount`, rendered key, display size, missing key 상태를 기록합니다. Phase 217 기준 검증 스크린샷은 `logs/phase217-cutscene-class-portrait.png`입니다.

#### Phase 218: Dungeon boss name icon

`DungeonScene` 보스 preview 이름 라벨은 `skill_ek_explode.png`를 `dungeon_boss_name_icon_*` `18x18` 이미지로 먼저 렌더합니다. 아이콘이 로드되면 보스 이름 텍스트는 `기억 흡수자`처럼 순수 텍스트만 사용하고, 기존 `★ 기억 흡수자 ★` 문자열은 texture 누락 시 fallback으로만 남깁니다.

`?debugScene=dungeon&renderer=canvas&dungeonFrameQa=boss` 경로는 보스 경고 후 보스 preview까지 생성하고, `aeternaDungeonFrameQa.bossNameIcon`에 rendered count, display size, fallback, missing key, legacy star 상태를 기록합니다. Phase 218 기준 검증 스크린샷은 `logs/phase218-dungeon-boss-name-icon.png`입니다.

#### Phase 221: Dungeon failure title icons

`DungeonScene` 패배/시간초과 중앙 타이틀은 기존 `💀`/`⏰` 텍스트 prefix 대신 Aseprite icon을 먼저 렌더합니다. 전멸은 `status_curse.png` / `status_curse_icon`, 시간초과는 `skill_tg_stop.png` / `skill_tg_stop_icon`을 `dungeon_failure_title_icon` `26x26` 이미지로 표시하고, icon texture가 있으면 label은 `패배...`, `시간 초과!`처럼 텍스트만 사용합니다. 기존 `composeDungeonGameOverText()`의 lead 문자열은 SSOT 호환을 위해 그대로 보존하고, `DungeonScene` 표시 계층에서만 glyph를 제거합니다.

`?debugScene=dungeon&renderer=canvas&dungeonFrameQa=defeat|timeout` 경로는 `aeternaDungeonFrameQa.failureTitleIcon`, `failureTitleLegacyGlyphPresent`, `missingFailureTitleIconKeys`, `failureLabel`에 실패 타이틀 아이콘 렌더 상태를 기록합니다. Phase 221 기준 검증 스크린샷은 `logs/phase221-dungeon-defeat-failure-title-icon.png`, `logs/phase221-dungeon-timeout-failure-title-icon.png`입니다.
