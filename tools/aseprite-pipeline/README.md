...
16. GameScene HUD 퀘스트 action hint를 objective kind별 Aseprite icon으로 표시
...
캐릭터 스프라이트는 `assets/source/aseprite/character/character-sprite-roster.json`을 기준으로 검증한다. 현재 런타임 대상은 6개 기본 클래스 전체의 5방향 full motion이다. 모든 기본 클래스 source `.aseprite`는 64x64 프레임 캔버스, 200프레임, export sheet `2560x320` 규격이며 character export는 `sheetColumns: 40`을 사용해 방향별 40프레임을 한 행에 배치한다. Motion tag는 `idle`, `walk`, `attack_melee`, `cast`, `hit`, `death`, `ready`, `victory` x `D`, `DL`, `L`, `UL`, `U` 40개다.
...
런타임 계약은 `client/src/assets/characterSpriteManifest.ts`가 가진다. `GameScene`, `BattleScene`, `DungeonScene`은 해당 manifest를 읽어 `load.spritesheet(..., { frameWidth: 64, frameHeight: 64 })`로 로드하고, 생성 후 `setFrame(0)`을 적용해 2560x320 sheet 전체 렌더링을 피한다. `GameScene`은 로컬 플레이어 선택 class와 `CHARACTER_SPRITE_MANIFEST` 전체를 preload해 `world:playerJoined.characterClass`가 있는 원격 플레이어도 Aseprite sheet frame `0`으로 표시한다. 로컬 class spritesheet가 없으면 class id별 `char_battle_<classId>.png` 전투 썸네일을 physics sprite texture로 먼저 사용하고, thumbnail texture가 없을 때만 legacy `player_sprite` front illustration/절차 texture fallback으로 내려간다. `?debugScene=game&renderer=canvas&localPlayerFallbackQa=1`은 manifest sheet를 의도적으로 건너뛰고 `aeternaGameLocalPlayerFallbackQa`에 thumbnail key/path, 표시 크기, legacy fallback 여부를 기록한다. 원격 class spritesheet가 없으면 같은 전투 썸네일을 `40x60` image로 먼저 표시하고, thumbnail texture가 없을 때만 기존 `40x56` 원격 플레이어 사각형 fallback을 사용한다. `?debugScene=game&renderer=canvas&remotePlayerFallbackQa=1`은 manifest sheet를 의도적으로 건너뛰고 `aeternaGameRemotePlayerFallbackQa`에 thumbnail key/path, 표시 크기, rectangle fallback 여부를 기록한다. `DungeonScene`은 manifest sheet를 먼저 표시하고, sheet가 없을 때는 class id별 `char_battle_<classId>.png` 전투 썸네일을 `56x84` image로 표시한 뒤 legacy `dungeon_player` side illustration과 절차 사각형을 안전 fallback으로만 사용한다. `?debugScene=dungeon&renderer=canvas&dungeonFrameQa=ready&dungeonPlayerThumbnailFallbackQa=1`은 manifest sheet를 의도적으로 건너뛰고 `aeternaDungeonPlayerThumbnailFallbackQa`에 thumbnail key/path, 표시 크기, legacy fallback 여부를 기록한다.

`BattleScene` 아군도 manifest sheet를 먼저 표시하고, sheet가 없으면 class id별 `char_battle_<classId>.png` 전투 썸네일을 사용한다. 미등록 class처럼 manifest와 전투 썸네일이 모두 없으면 공용 Aseprite placeholder `client/public/assets/generated/ui/placeholders/placeholder.png`를 `48x64` image로 먼저 표시하고, placeholder texture까지 없을 때만 기존 파란 절차 사각형 fallback으로 내려간다. `?debugScene=battle&renderer=canvas&battleAllyFallbackQa=1&class=missing_class`은 `aeternaBattleAllyFallbackQa`에 fallback image key/name, 표시 크기, rectangle fallback 여부, 누락 key를 기록한다.
...
6개 기본 클래스 5방향 제작은 `D`, `DL`, `L`, `UL`, `U` 각각에 `idle`, `walk`, `attack_melee`, `cast`, `hit`, `death`, `ready`, `victory` tag를 포함한다. 다음 확장 단계는 클래스별 motion 품질 보정, 방향별 silhouette 차별화, combo/skill 전용 pose 추가다.
...
필드 NPC id가 `spriteResourceManifest`와 legacy `NPC_SPRITE_MAP` 양쪽에 없으면 `GameScene.preload()`는 공용 Aseprite placeholder PNG `client/public/assets/generated/ui/placeholders/placeholder.png`를 `placeholder` key로 로드한다. `_spawnNpc()`는 이 texture가 있을 때 `48x64` image를 먼저 표시하고, placeholder texture까지 없을 때만 기존 `32x48` 절차 사각형 fallback을 생성한다. `?debugScene=game&renderer=canvas&fieldNpcFallbackQa=1`은 `aeternaGameFieldNpcFallbackQa`에 rendered count, image key, display size, procedural rectangle 여부, 누락 key를 기록한다.
...
- `GameScene.preload()`도 같은 runtime PNG를 `GAME_SCENE_FIELD_MONSTER_FALLBACK_TEXTURES`로 로드한다. 필드 몬스터 sprite manifest가 없는 경우 `battle_monster_fallback`은 `56x56`, `battle_boss_fallback`은 `72x72` 이미지로 먼저 표시하고, PNG key가 없을 때만 색 사각형/이모지 fallback을 생성한다.
- `?debugScene=game&renderer=canvas&fieldMonsterFallbackQa=1`은 미등록 일반/보스 몬스터 2개를 스폰하고 `aeternaGameFieldMonsterFallbackQa`에 fallback image count, 표시 크기, legacy emoji 여부, 누락 key를 기록한다.
...
- `DungeonScene` 플레이어 fallback은 full character spritesheet를 먼저 사용하고, sheet가 없을 때 현재 class id의 `char_battle_<classId>.png` Aseprite battle thumbnail을 `dungeon_player_thumbnail_fallback` `56x84` image로 먼저 렌더한다. Thumbnail texture가 없을 때만 legacy `dungeon_player` side illustration과 rectangle fallback을 사용한다. `?debugScene=dungeon&renderer=canvas&dungeonFrameQa=ready&dungeonPlayerThumbnailFallbackQa=1`는 `aeternaDungeonPlayerThumbnailFallbackQa`에 thumbnail 렌더 상태, 표시 크기, legacy fallback 여부, 누락 key를 기록한다.
...
- `DungeonScene` 보스 경고 연출은 `skill_ek_explode.png` / `skill_ek_explode_icon`을 `dungeon_boss_warning_icon` `30x30` image로 먼저 표시한다. 보스 preview 이름 라벨도 같은 icon을 `dungeon_boss_name_icon_*` `18x18` image로 먼저 표시하고, icon texture가 있으면 이름 text는 `기억 흡수자`처럼 text-only로 유지한다. texture 누락 시에만 기존 `⚠ WARNING ⚠\n보스 등장!` 경고 label과 `★ <보스명> ★` 이름 label fallback을 사용한다. `?debugScene=dungeon&renderer=canvas&dungeonFrameQa=boss`는 보스 경고 뒤 보스 preview까지 생성해 `aeternaDungeonFrameQa.bossWarningIcon`, `bossNameIcon`, `bossWarningLegacyGlyphPresent`, `bossNameLegacyGlyphPresent`, `missingBossWarningIconKeys`, `missingBossNameIconKeys`에 렌더 상태를 기록한다.
...
- `BattleScene` 보스 강공 예고 표식은 `skill_ek_explode.png` / `skill_ek_explode_icon`을 `battle_boss_telegraph_icon` `30x30` Aseprite image로 먼저 표시한다. texture 누락 시에만 기존 `⚠` text fallback을 사용한다. `?debugScene=battle&renderer=canvas&battleBossTelegraphIconQa=1`는 첫 enemy를 deterministic boss로 지정하고 `document.body.dataset.aeternaBattleBossTelegraphIconQa`에 렌더 texture key, 표시 크기, fallback 여부, legacy glyph 존재 여부, 누락 key를 기록한다.
...
- `SettingsScene.preload()`는 `UI-SET-002-DEF`, `UI-SET-003-DEF`, `UI-SET-004-DEF`를 설정 본문, 키바인드, 하단 액션 panel frame으로 preload하고, `UI-BTN-006-DEF`를 `ui_frame_settings_action_button` key로 preload해 피드백/뒤로가기 액션 버튼 frame 2개를 먼저 표시한다. `UI-BTN-005-DEF`는 `ui_frame_settings_slider_track` key로 preload해 BGM/SFX/자막 배경 불투명도 slider track frame 3개를 먼저 표시한다. 설정/사운드/언어/접근성/키바인드 섹션 제목은 `settings_title.png`, `settings_sound.png`, `settings_language.png`, `settings_accessibility.png`, `settings_keybind.png`를 각각 `ui_icon_settings_*` texture로 preload하고, title은 `22x22`, 일반 섹션은 `18x18` image로 먼저 표시한다. 제목 text는 `설정`, `사운드`, `언어`, `접근성`, `키바인드`만 유지하며 texture 누락 시에만 기존 `⚙`/`🔊`/`🌐`/`♿`/`⌨` fallback label을 사용한다. `skill_mw_arrow.png`와 `skill_tg_reverse.png`는 피드백/뒤로가기 버튼 내부 `18x18` action icon으로 먼저 표시하고, 설정 항목 focus marker는 `skill_mw_arrow.png`를 `settings_focus_icon` `14x14` image로 재사용한다. texture가 있으면 slider/toggle/cycle label에는 `▶` prefix를 넣지 않고, texture 누락 시에만 legacy prefix fallback을 사용한다. fill bar, 값 갱신, 버튼 label/callback은 Phaser 동적 UI로 유지하며, `settingsFrameQa=1`은 `aeternaSettingsFrameQa.settingsSectionIcon`, `settingsSectionLabelLegacyGlyphPresent`, `missingSectionIconKeys`, `actionIcon`, `settingsFocusIcon`, `settingsFocusLabelLegacyGlyphPresent`, `missingSettingsFocusIconKeys`에 아이콘 렌더 상태를 기록한다.
...
- `LobbyScene.preload()`와 `SkillTreeUI`는 `UI-SET-002-DEF`, `UI-SET-003-DEF`를 스킬 트리 main/detail panel frame으로 preload/render하고, `UI-BTN-006-DEF`를 `ui_frame_skill_tree_action_button` key로 preload해 main close/reset 및 detail unlock/upgrade/close action button frame을 먼저 표시한다. `LobbyScene.preload()`는 `preloadSkillTreeIconResources()`로 6개 클래스의 스킬 트리 node icon 후보를 함께 로드하고, `preloadSkillTreeAdvancedIllustrations()`로 현재 클래스의 전직 일러스트 `char_<class>_adv1`~`char_<class>_adv3`도 함께 로드한다. `SkillTreeUI`는 각 node의 `skill.icon` 또는 tier fallback을 `spriteResourceManifest` skill icon으로 해석해 `48x48` Aseprite image를 먼저 표시하고, 전직 일러스트는 `skill_tree_advanced_illustration_<class>_<n>` 이름의 `52x68` image 3장으로 상단에 표시한다. 제목 아이콘은 현재 클래스의 1티어 skill icon을 `20x20` image로 먼저 표시하고, 리셋 action button 내부 아이콘은 `skill_tg_reverse.png` / `skill_tg_reverse_icon`을 `18x18` image로 먼저 표시하며, 메인/상세 닫기 action button 내부 아이콘도 같은 리소스를 `16x16` image로 먼저 표시한다. 상세 패널의 분기/잠김 라인은 `skill_mw_storm.png` / `skill_mw_storm_icon`, `skill_tg_stop.png` / `skill_tg_stop_icon`을 `14x14` image로 먼저 표시한다. texture 누락 시에만 `>`, ASCII `x`, 또는 각 UI별 안전 label fallback을 사용한다. class color stroke, 선택 강조, 버튼 label/callback은 Phaser primitive와 동적 text로 유지하며, `?debugScene=lobby&renderer=canvas&skillTreeQa=1`는 `document.body.dataset.aeternaSkillTreeFrameQa`에 panel/action button frame 렌더 상태와 `advancedIllustration`, `titleIcon`, `skillNodeIcon`, `resetActionIcon`, `closeActionIcon`, `detailLineIcon` expected/rendered count, texture key, fallback state, 누락 key를 기록한다.
- `CharacterSelectScene.preload()`는 `UI-INV-001-DEF`와 `UI-INV-002-DEF`를 기존 캐릭터 row와 클래스 생성 card frame으로 preload하고, `UI-BTN-006-DEF`를 `ui_frame_character_select_name_input` 및 `ui_frame_character_select_action_button` key로 preload해 캐릭터명 DOM input CSS background와 생성 액션 버튼 frame을 먼저 표시한다. 클래스 생성 card avatar는 class id별 `char_battle_*.png` battle thumbnail을 `54x81` image `character_select_class_card_avatar_<classId>`로 먼저 표시하고, texture 누락 시에만 기존 front illustration과 색 원형 fallback으로 후퇴한다. 기존 캐릭터 row avatar도 같은 battle thumbnail을 `28x42` image로 먼저 표시하고, texture 누락 시에만 기존 색 원형 fallback을 사용한다. 로그아웃 버튼은 `skill_tg_reverse.png` / `skill_tg_reverse_icon`을 `character_select_logout_icon` `16x16` image로 먼저 표시하고 label은 `로그아웃` 텍스트만 유지하며, texture 누락 시에만 기존 `← 로그아웃` fallback을 사용한다. `_addCharacterSelectFrame()`은 로드된 card/button frame을 먼저 표시하고, `_createNameInput()`은 canvas CSS scale 기준으로 input 위치를 보정하며, `characterSelectFrameQa=1`은 `aeternaCharacterSelectFrameQa.classCardAvatar`, `logoutIcon`, `missingLogoutIconKeys`에 렌더 상태를 기록한다. `characterSelectExistingQa=1`을 추가하면 `existingCharacterAvatar`도 함께 검증한다.
...
- `CutsceneScene.preload()`는 `UI-HUD-006-DEF`를 하단 대화 box frame으로, `UI-BTN-006-DEF`를 `ui_frame_cutscene_action_button` key의 스킵/다음 버튼 frame으로 preload하고, 내레이터 `npc_portrait_18_memory_fragment_portrait.png`와 기본 클래스 6종 `char_illust_*_front.png`도 컷씬 portrait texture로 preload한다. `_addCutsceneFrame()`/`_addCutsceneActionButton()`은 로드된 frame을 먼저 표시한다. 스킵/다음 버튼 내부 아이콘은 `skill_tg_haste.png`와 `skill_mw_arrow.png`를 `16x16` Aseprite image로 먼저 표시하고, narrator/class portrait는 `96x96` Aseprite image로 먼저 표시한다. texture 누락 시에만 기존 `[ 스킵 ]`, `다음 ▶` label fallback 또는 무표시 portrait fallback을 사용한다. `?debugScene=cutscene&renderer=canvas&cutsceneFrameQa=1&cutsceneSpeaker=ether_knight`는 `document.body.dataset.aeternaCutsceneFrameQa`에 dialogue box/action button frame, `actionButtonIcon`, `portrait`, `missingCutscenePortraitKeys` 렌더 상태를 기록한다.
...
- `ChatUI`는 `UI-HUD-008-DEF`를 독립 채팅 panel frame으로, `UI-BTN-006-DEF`를 입력창 frame, 채널 탭 button frame, 이모지 button frame으로 preload/render한다. 이모지 button 내부 아이콘은 `status_charm.png` / `status_charm_icon`을 `16x16` Aseprite image로 먼저 표시하고, 시스템 메시지 prefix는 `chat_system.png` / `ui_icon_chat_system`을 `14x14` Aseprite image로 먼저 표시한다. texture 누락 시에만 기존 `😀` 또는 `⚙️` glyph fallback을 사용한다. `?debugScene=chat&renderer=canvas&chatFrameQa=1`는 `document.body.dataset.aeternaChatFrameQa`에 panel/input/tab/emoji frame 렌더 상태와 `emojiButtonIcon`, `systemMessageIcon`, `systemMessageLegacyGlyphPresent`, `missingSystemMessageIconKeys` image/fallback 상태를 기록한다.
...
- `ComboUI`는 `UI-BTN-005-DEF`를 `ui_frame_combo_chain_gauge` key로 preload하고, 우측 상단 chain gauge track frame을 먼저 표시한다. 콤보 달성 라벨은 `skill_mw_storm.png` / `skill_mw_storm_icon`을 `28x28` Aseprite image로 먼저 표시하고, texture가 있으면 label은 `전격 강타!`처럼 텍스트만 사용한다. texture 누락 시에만 기존 `🔥 전격 강타!` fallback을 유지한다. 콤보 힌트 행의 구분자는 `skill_mw_arrow.png` / `skill_mw_arrow_icon`을 `14x14` Aseprite image로 먼저 표시하고, texture 누락 시에만 기존 `→` fallback을 유지한다. hit counter, multiplier, gauge fill 색상/감소, hint row text는 동적 Phaser UI로 유지하며, `?debugScene=combo&renderer=canvas&comboFrameQa=1`는 `document.body.dataset.aeternaComboFrameQa`에 chain gauge frame, hit count, gauge fill, hint 렌더 상태와 `comboAchievedIcon`, `comboTextLegacyGlyphPresent`, `missingComboAchievedIconKeys`, `hintSeparatorIcon`, `hintTextLegacyArrowPresent`, `missingHintSeparatorIconKeys`를 기록한다.
- `tests/unit/uiFrameAssets.test.ts`가 runtime 폴더 전체의 파일 수, PNG 규격, 로스터 연결, BattleScene/BattleUI 전투 슬롯/로그 패널/스킬 tooltip/utility button, pause/resume/flee utility icon texture 상태와 하단 HUD/커맨드/서브메뉴/결과/패배 팝업/페이싱/협공 버튼 연결, GameScene/HudOverlay 필드 HUD DOM panel/button/quickslot icon/status avatar 연결, DungeonScene 상태/title/action/보상 frame 연결, LobbyScene 미니맵/하단 nav icon/주요 모달/NPC portrait 연결, Minimap standalone panel 및 플레이어/dynamic marker image 연결, MinimapOverlay 독립 panel 연결, NavigationManager 방향 화살표 image 연결, DialogueBox 독립 panel/choice button/next indicator icon 연결, ChatUI 독립 panel/input/tab/emoji button/emoji icon/system message icon 연결, TutorialFlowManager 안내 panel/skip button 연결, TutorialManager DOM panel/action button/next button icon 연결, Transition loading panel/spinner track 연결, CoachmarkOverlay panel/action button/action icon 연결, ComboUI chain gauge 연결, WorldScene title icon/선택 패널 연결, LoadingScene panel/progress track/tip icon 연결, SettingsScene panel/action button/action icon/focus marker 연결, FeedbackForm panel/button/title/submit/close/type icon 연결, SkillTreeUI panel/detail/action button/node icon/reset action icon 연결, CharacterSelectScene card/name input/action button/existing avatar/logout icon 연결, CutsceneScene dialogue box/action button/narrator/class portrait 연결을 검증한다. `tests/unit/spriteResourceManifest.test.ts`는 BattleScene 마법/아이템 서브메뉴 focus marker가 `skill_mw_arrow.png` image를 먼저 사용하고 `▶` prefix는 fallback으로만 남는 계약을 추가로 검증한다.
...
- `GameScene` 필드 NPC 누락 리소스도 같은 `placeholder` PNG를 재사용한다. NPC spritesheet와 legacy field NPC texture가 모두 없을 때 `48x64` image fallback을 먼저 렌더하고, PNG key가 없을 때만 절차 사각형 fallback으로 내려간다.
...
- BattleUI 전투 로그 하이라이트는 전투 시작 `skill_ek_slash.png`, 크리티컬/보스 강공 준비/보스 강공 피해 `skill_ek_explode.png`, CHAIN/콤보/ECHO/스킬 발동/2인 협공 `skill_mw_storm.png`, MP 부족 `skill_mw_passive.png`, 포션 회복 `ITM-CON-001.png`, 쿨다운/대기/재연결 복구/ATB 모드/도주 실패/도주 차단/도주 불가 `skill_tg_stop.png`, 자동 전투 모드 `skill_tg_haste.png`, BGM 재생/누락 `battle_bgm_playing.png`/`battle_bgm_missing.png` UI icon, 승리/3인 협공 `skill_ek_ultimate.png`, 레벨 업 `skill_ek_passive.png`, 도주 성공/비상 도주 `skill_vw_warp.png`, 방어/반사 `status_shield.png`, 사망/패배 `status_curse.png` Aseprite icon을 재사용한다. 런타임 표시는 `16x16`이며, icon texture가 있으면 하이라이트 label은 텍스트만 사용하고 `⚔`, `💥`, `🔥`, `🎉`, `🆙`, `⚡`, `✨`, `🌟`, `🔁`, `🏆`, `🛡`, `💀`, `💔`, `💢`, `💧`, `⏳`, `⏭`, `🧪`, `🔌`, `⚙`, `⏱`, `🎵`, `🔇`, `🏃`, `❌`, `🚧`, `🔒`, `🆘` glyph 문자열은 fallback에서만 유지한다.
- BattleUI 도주 로그 하이라이트 QA는 `battleLogHighlightIconQa=escapeSuccess|escapeFail|escapeBlocked|escapeForbidden|escapeCritical`을 사용한다. `composeEscapeLog()`와 QA source message는 legacy escape glyph를 보존하고, runtime display text는 `도주 성공!`/`도주 실패!`처럼 glyph를 제거한다. QA payload는 `escapeLegacyGlyphPresent`를 별도로 기록한다.
...
- BattleScene 속성 데미지 태그는 화염 `skill_ek_explode.png`, 얼음 `skill_mw_storm.png`, 번개 `skill_mw_bolt.png`, 그림자 `skill_mw_ultimate.png`, 신성 `skill_mw_heal.png` Aseprite skill icon을 재사용한다. 런타임 표시는 `16x16`이며, icon texture가 있으면 label은 `화염`, `얼음`, `번개`, `그림자`, `신성` 텍스트만 사용하고 `🔥`, `❄`, `⚡`, `🌑`, `✨` glyph 문자열은 fallback에서만 유지한다. `battleElementTagIconQa=fire|ice|lightning|shadow|holy`는 이 계약을 기록한다.
...
- LobbyScene 마을 제목은 `zone_aether_plains.png` Aseprite worldmap icon을 재사용한다. 런타임 표시는 `20x20`이며, icon texture가 있으면 title label은 `아에테리아 마을` 텍스트만 사용하고 `☆` glyph 문자열은 fallback에서만 유지한다. `lobbyTitleIconQa=1`은 `aeternaLobbyTitleIconQa`에 렌더 수, 표시 크기, legacy glyph, missing key 상태를 기록한다.
- LobbyScene NPC action 알림은 상점 `ITM-CON-001.png`, 강화 `ITM-MAT-001.png`, 퀘스트 `ITM-QST-004.png`, 파티 `skill_ek_slash.png`, 스토리 `ITM-QST-001.png` Aseprite icon을 재사용한다. 런타임 표시는 `18x18`이며, icon texture가 있으면 알림 label은 `상인 미라: 상점을 열었습니다.`처럼 텍스트만 사용하고 `🛒`/`🔨`/`📜`/`⚔`/`📖` glyph 문자열은 fallback에서만 유지한다. `lobbyNotificationIconQa=shop|enhance|quest|party|story`는 `aeternaLobbyNotificationIconQa`에 렌더 수, 표시 크기, legacy glyph, missing key 상태를 기록한다.
...
- DungeonScene 클리어 보상 패널은 EXP 표식 `skill_ek_passive.png`와 Gold 표식 `ITM-MAT-002.png` Aseprite icon을 재사용한다. 런타임 표시는 각각 `18x18`이며, 보상 값 텍스트와 return action은 기존 동적 UI로 유지한다. `dungeonFrameQa=clear`는 `rewardIcon`, `missingRewardIconKeys`, `rewardLabelLegacyGlyphPresent`로 이 계약을 기록한다.
- DungeonScene 몬스터 preview 누락 fallback은 일반 `battle_monster_fallback.png`, 보스 `battle_boss_fallback.png` Aseprite image를 재사용한다. 런타임 표시는 일반 `56x56`, 보스 `80x80`이며, wave spawn, name label, battle launch flow는 기존 동적 UI로 유지한다. `dungeonMonsterFallbackQa=1`은 `aeternaDungeonMonsterFallbackQa`에 fallback image 렌더 수, texture key, legacy emoji fallback 상태를 기록한다.
- DungeonScene 보스 preview 이름 라벨은 `skill_ek_explode.png` Aseprite skill icon을 재사용한다. 런타임 표시는 `18x18`이며, icon texture가 있으면 보스명 text에는 `★` 장식을 넣지 않는다. `dungeonFrameQa=boss`는 `bossNameIcon`, `bossNameLegacyGlyphPresent`, `missingBossNameIconKeys`, `bossNameLabels`로 이 계약을 기록한다.
- DungeonScene 실패/시간초과 타이틀은 전멸 `status_curse.png` Aseprite status icon과 시간초과 `skill_tg_stop.png` Aseprite skill icon을 재사용한다. 런타임 표시는 `26x26`이며, icon texture가 있으면 label은 `패배...`, `시간 초과!` 텍스트만 사용하고 `💀`/`⏰` glyph 문자열은 fallback에서만 유지한다. `dungeonFrameQa=defeat|timeout`은 `failureTitleIcon`, `failureTitleLegacyGlyphPresent`, `missingFailureTitleIconKeys`, `failureLabel`로 이 계약을 기록한다.
- GameScene 로컬 플레이어 spritesheet 누락 fallback은 현재 class id의 `char_battle_<classId>.png` Aseprite battle thumbnail을 재사용한다. 런타임 표시는 원본 `64x96` physics sprite이며, movement/camera follow/input은 기존 플레이어 객체를 유지한다. `localPlayerFallbackQa=1`은 `aeternaGameLocalPlayerFallbackQa`에 texture key, display size, legacy fallback 상태를 기록한다.
- GameScene 원격 플레이어 spritesheet 누락 fallback도 현재 class id의 `char_battle_<classId>.png` Aseprite battle thumbnail을 재사용한다. 런타임 표시는 `40x60` image이며, remote name tag와 entity lifecycle은 기존 동적 로직을 유지한다. `remotePlayerFallbackQa=1`은 `aeternaGameRemotePlayerFallbackQa`에 thumbnail 렌더 수, texture key, rectangle fallback 상태를 기록한다.
- BattleScene 아군 spritesheet/thumbnail 누락 fallback은 공용 `placeholder.png` Aseprite image를 재사용한다. 런타임 표시는 `48x64` image이며, ATB/targeting/name tag 흐름은 기존 아군 객체를 유지한다. `battleAllyFallbackQa=1&class=missing_class`은 `aeternaBattleAllyFallbackQa`에 fallback image 렌더 수, texture key, rectangle fallback 상태를 기록한다.
- CharacterSelectScene 클래스 생성 card avatar는 class id별 `char_battle_<classId>.png` Aseprite battle thumbnail을 재사용한다. 런타임 표시는 `54x81` image이며, class card frame, hover/select stroke, name/stat text, DOM name input은 기존 동적 로직을 유지한다. `characterSelectFrameQa=1`은 `aeternaCharacterSelectFrameQa.classCardAvatar`에 6개 thumbnail 렌더 수, texture key, fallback 상태를 기록한다.
