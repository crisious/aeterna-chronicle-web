...
const CREATE_SCRIPT = path.join(TOOL_DIR, 'scripts', 'create-ui-frame.lua');
const PALETTE_PATH = 'assets/source/aseprite/palettes/aeterna-core.gpl';
const CATEGORY = 'uiFrame';
const SOURCE_ROOT = 'assets/source/aseprite/uiFrame';
const RUNTIME_DIR = 'client/public/assets/generated/ui/frames';
const RUNTIME_KEY_PREFIX = 'ui_frame';

export const BUILD_UI_FRAME_LIBRARY_USAGE =
  'Usage: node tools/aseprite-pipeline/build-ui-frame-library.mjs [--ids <comma-separated-ui-frame-ids>]';
...
function readRuntimeFrameIds(runtimeDir = RUNTIME_DIR) {
...
function parseFrameId(frameId) {
  const match = /^UI-([A-Z]+)-(\d{3})-([A-Z]+)$/u.exec(frameId);

  if (!match) {
    throw new Error(`UI frame id must match UI-XXX-###-THEME: ${frameId}`);
  }
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite UI frame source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}

function createBuildTarget(frameId) {
  const { family, theme } = parseFrameId(frameId);
  const source = toPosixPath(path.join(SOURCE_ROOT, theme, family, `${frameId}.aseprite`));
  const generatedPng = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${frameId}.png`));
  const generatedJson = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${frameId}.json`));
  const runtimePng = toPosixPath(path.join(RUNTIME_DIR, `${frameId}.png`));

  return {
    id: `${RUNTIME_KEY_PREFIX}_${frameId}`,
    frameId,
    category: CATEGORY,
    source,
    generatedPng,
    ...
    generatedJson,
    runtimePng,
    runtimeKey: `${RUNTIME_KEY_PREFIX}_${frameId}`,
  };
}
...
export function parseBuildUiFrameLibraryArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated UI frame id list after --ids.');
      }
...
export function buildUiFrameLibrary({ ids = null, dependencies = {} } = {}) {
  ...
  const frameIds = ids?.length ? [...new Set(ids)].sort((a, b) => a.localeCompare(b)) : readRuntimeFrameIds();

  if (frameIds.length === 0) {
    throw new Error('No UI frame ids found.');
  }

  const targets = [];

  for (const frameId of frameIds) {
    const target = createBuildTarget(frameId);
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildUiFrameLibraryArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_UI_FRAME_LIBRARY_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildUiFrameLibrary(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
