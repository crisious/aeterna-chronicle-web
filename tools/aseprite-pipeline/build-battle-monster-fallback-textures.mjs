...
const CREATE_SCRIPT = path.join(TOOL_DIR, 'scripts', 'create-battle-monster-fallback-texture.lua');
const PALETTE_PATH = 'assets/source/aseprite/palettes/aeterna-core.gpl';
const RUNTIME_ROOT = 'client/public/assets/generated/monsters/fallback';

const BATTLE_MONSTER_FALLBACK_TEXTURES = [
  {
    textureId: 'battle_monster_fallback',
    category: 'battleMonsterFallbackTexture',
    size: 60,
    runtimeKey: 'battle_monster_fallback',
  },
  {
    textureId: 'battle_boss_fallback',
...
export const BUILD_BATTLE_MONSTER_FALLBACK_TEXTURES_USAGE =
  'Usage: node tools/aseprite-pipeline/build-battle-monster-fallback-textures.mjs [--ids <comma-separated-texture-ids>]';
...
function createBuildTarget(texture) {
  ...
  return {
    id: `battle_monster_fallback_texture_${texture.textureId}`,
    ...
    battleMonsterFallbackTextureId: texture.textureId,
    size: texture.size,
  };
}
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      '--script-param',
      `texture=${target.battleMonsterFallbackTextureId}`,
      '--script-param',
      `size=${target.size}`,
      '--script',
      CREATE_SCRIPT,
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite battle monster fallback source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}
...
function createRosterEntry(target, priority) {
  return {
    ...
    requiredTags: [],
    battleMonsterFallbackTextureId: target.battleMonsterFallbackTextureId,
  };
}
...
export function parseBuildBattleMonsterFallbackTexturesArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated battle monster fallback texture id list after --ids.');
      }
...
export function buildBattleMonsterFallbackTextures({ ids = null, dependencies = {} } = {}) {
  ...
  const textureById = new Map(BATTLE_MONSTER_FALLBACK_TEXTURES.map((texture) => [texture.textureId, texture]));
  const selectedTextures = ids?.length
    ? [...new Set(ids)].sort((a, b) => a.localeCompare(b)).map((id) => {
      const texture = textureById.get(id);
      if (!texture) {
        throw new Error(`Unknown battle monster fallback texture id: ${id}`);
      }
      return texture;
    })
    : BATTLE_MONSTER_FALLBACK_TEXTURES;
  ...
  for (const texture of selectedTextures) {
    ...
    normalizeFile(
      ...
      target.battleMonsterFallbackTextureId,
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildBattleMonsterFallbackTexturesArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_BATTLE_MONSTER_FALLBACK_TEXTURES_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildBattleMonsterFallbackTextures(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
