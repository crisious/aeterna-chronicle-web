...
const CREATE_SCRIPT = path.join(TOOL_DIR, 'scripts', 'create-npc-sprite.lua');
const PALETTE_PATH = 'assets/source/aseprite/palettes/aeterna-core.gpl';
const CATEGORY = 'npcSprite';
const SOURCE_ROOT = 'assets/source/aseprite/npcSprite';
const RUNTIME_DIR = 'client/public/assets/generated/characters/npc_sprites';

export const BUILD_NPC_SPRITE_LIBRARY_USAGE =
  'Usage: node tools/aseprite-pipeline/build-npc-sprite-library.mjs [--ids <comma-separated-npc-sprite-ids>]';
...
function parseNpcId(spriteId) {
  const match = /^(.+)_sprite$/u.exec(spriteId);

  if (!match) {
    throw new Error(`NPC sprite id must match <npc-id>_sprite: ${spriteId}`);
  }

  return match[1];
}

function isNpcSpriteRuntimePng(runtimePath) {
  const size = readPngSize(resolveRepoPath(runtimePath));

  return size.w === 256 && size.h === 384;
}

function readRuntimeSpriteIds(runtimeDir = RUNTIME_DIR) {
  return readdirSync(resolveRepoPath(runtimeDir))
    .filter((fileName) => fileName.endsWith('.png'))
    .map((fileName) => fileName.replace(/\.png$/u, ''))
    .filter((spriteId) => /^[0-9]{2}_.+_sprite$/u.test(spriteId))
    .filter((spriteId) => isNpcSpriteRuntimePng(toPosixPath(path.join(runtimeDir, `${spriteId}.png`))))
...
function normalizeExplicitId(value) {
  ...
  if (id.length === 0) {
    throw new Error(`Invalid NPC sprite selector: ${value}`);
  }

  return id.endsWith('_sprite') ? id : `${id}_sprite`;
}

function createBuildTarget(spriteId) {
  const npcId = parseNpcId(spriteId);
  const source = toPosixPath(path.join(SOURCE_ROOT, npcId, `${spriteId}.aseprite`));
  const generatedPng = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${spriteId}.png`));
  const generatedJson = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${spriteId}.json`));
  const runtimePng = toPosixPath(path.join(RUNTIME_DIR, `${spriteId}.png`));
  const runtimeKey = spriteId;

  return {
    id: `npc_sprite_${npcId}`,
    spriteId,
    npcId,
    category: CATEGORY,
    source,
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite NPC sprite source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}
...
export function parseBuildNpcSpriteLibraryArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated NPC sprite id list after --ids.');
      }
...
export function buildNpcSpriteLibrary({ ids = null, dependencies = {} } = {}) {
  ...
  const runtimeIds = readRuntimeSpriteIds();
  ...
  if (missingIds.length > 0) {
    throw new Error(`Unknown 256x384 NPC sprite ids: ${missingIds.join(', ')}`);
  }

  if (selectedIds.length === 0) {
    throw new Error('No 256x384 NPC sprite runtime PNGs found.');
  }
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildNpcSpriteLibraryArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_NPC_SPRITE_LIBRARY_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildNpcSpriteLibrary(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
