...
const CATEGORY = 'characterSpriteSheet';
const SOURCE_ROOT = 'assets/source/aseprite/characterSpriteSheet';
const RUNTIME_DIR = 'client/public/assets/generated/characters/sprites';

export const BUILD_CHARACTER_SPRITE_SHEET_LIBRARY_USAGE =
  'Usage: node tools/aseprite-pipeline/build-character-sprite-sheet-library.mjs [--ids <comma-separated-sprite-sheet-ids>]';
...
function normalizeExplicitId(value) {
  return value.trim().replace(/\.png$/u, '').replace(/^character_sprite_sheet_/u, '');
}

function parseVariantAdvancement(variant) {
  if (variant === 'base') {
    return 0;
  }

  const advancedMatch = /^adv([1-9][0-9]*)$/u.exec(variant);
  if (advancedMatch) {
    return Number.parseInt(advancedMatch[1], 10);
  }

  if (variant === '2nd') return 1;
  if (variant === '3rd') return 2;
  if (variant === '4th') return 3;
...
function parseRuntimeFile(fileName) {
  ...
  return {
    spriteSheetId: fileName.replace(/\.png$/u, ''),
    classId: match[1],
    variant: match[2],
    advancement: parseVariantAdvancement(match[2]),
  };
}

function readRuntimeTargets() {
  return readdirSync(resolveRepoPath(RUNTIME_DIR))
    .filter((fileName) => fileName.endsWith('.png'))
    .map(parseRuntimeFile)
    .filter(Boolean)
    .sort((a, b) => a.spriteSheetId.localeCompare(b.spriteSheetId))
    .map(createBuildTarget);
}

function createBuildTarget(parsed) {
  const source = toPosixPath(path.join(SOURCE_ROOT, parsed.classId, `${parsed.spriteSheetId}.aseprite`));
  const generatedPng = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${parsed.spriteSheetId}.png`));
  const generatedJson = toPosixPath(path.join('assets/generated/aseprite', CATEGORY, `${parsed.spriteSheetId}.json`));
  const runtimePng = toPosixPath(path.join(RUNTIME_DIR, `${parsed.spriteSheetId}.png`));
  const runtimeKey = parsed.spriteSheetId;

  return {
    id: `character_sprite_sheet_${parsed.spriteSheetId}`,
    category: CATEGORY,
    source,
    generatedPng,
    ...
    runtimePng,
    runtimeKey,
    spriteSheetId: parsed.spriteSheetId,
...
function filterTargetsByIds(targets, ids) {
  ...
  for (const target of targets) {
    ...
    byId.set(target.spriteSheetId, target);
    byId.set(target.runtimeKey, target);
  }
  ...
  if (missingIds.length > 0) {
    throw new Error(`Unknown character sprite sheet ids: ${missingIds.join(', ')}`);
  }
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite character sprite sheet source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}
...
function createRosterEntry(target, priority) {
  return {
    ...
    requiredTags: [],
    characterSpriteSheetId: target.spriteSheetId,
...
export function parseBuildCharacterSpriteSheetLibraryArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated character sprite sheet id list after --ids.');
      }
...
export function buildCharacterSpriteSheetLibrary({ ids = null, dependencies = {} } = {}) {
  ...
  if (targets.length === 0) {
    throw new Error('No character sprite sheet runtime PNGs found.');
  }
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildCharacterSpriteSheetLibraryArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_CHARACTER_SPRITE_SHEET_LIBRARY_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildCharacterSpriteSheetLibrary(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
