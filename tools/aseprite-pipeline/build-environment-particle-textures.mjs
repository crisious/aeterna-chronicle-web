...
const CREATE_SCRIPT = path.join(TOOL_DIR, 'scripts', 'create-environment-particle-texture.lua');
const PALETTE_PATH = 'assets/source/aseprite/palettes/aeterna-core.gpl';
const RUNTIME_ROOT = 'client/public/assets/generated/vfx/particles';

const ENVIRONMENT_PARTICLE_TEXTURES = [
  {
...
export const BUILD_ENVIRONMENT_PARTICLE_TEXTURES_USAGE =
  'Usage: node tools/aseprite-pipeline/build-environment-particle-textures.mjs [--ids <comma-separated-texture-ids>]';
...
function createBuildTarget(texture) {
  ...
  return {
    id: `environment_particle_texture_${texture.textureId}`,
    ...
    environmentParticleTextureId: texture.textureId,
    width: texture.width,
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      '--script-param',
      `particle=${target.environmentParticleTextureId}`,
      '--script-param',
      `width=${target.width}`,
      '--script-param',
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite environment particle source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}
...
function createRosterEntry(target, priority) {
  return {
    ...
    requiredTags: [],
    environmentParticleTextureId: target.environmentParticleTextureId,
  };
}
...
export function parseBuildEnvironmentParticleTexturesArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated environment particle texture id list after --ids.');
      }
...
export function buildEnvironmentParticleTextures({ ids = null, dependencies = {} } = {}) {
  ...
  const textureById = new Map(ENVIRONMENT_PARTICLE_TEXTURES.map((texture) => [texture.textureId, texture]));
  const selectedTextures = ids?.length
    ? [...new Set(ids)].sort((a, b) => a.localeCompare(b)).map((id) => {
      const texture = textureById.get(id);
      if (!texture) {
        throw new Error(`Unknown environment particle texture id: ${id}`);
      }
      return texture;
    })
    : ENVIRONMENT_PARTICLE_TEXTURES;
  ...
  for (const texture of selectedTextures) {
    ...
    normalizeFile(
      ...
      target.environmentParticleTextureId,
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildEnvironmentParticleTexturesArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_ENVIRONMENT_PARTICLE_TEXTURES_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildEnvironmentParticleTextures(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
