...
const CREATE_SCRIPT = path.join(TOOL_DIR, 'scripts', 'create-effect-fallback-texture.lua');
const PALETTE_PATH = 'assets/source/aseprite/palettes/aeterna-core.gpl';
const RUNTIME_ROOT = 'client/public/assets/generated/vfx/fallback';

const EFFECT_FALLBACK_TEXTURES = [
  {
    textureId: 'hit_fallback_slash',
    category: 'effectFallbackTexture',
    size: 32,
    runtimeKey: 'hit_fallback_slash',
  },
  {
    textureId: 'hit_fallback_blunt',
    category: 'effectFallbackTexture',
    size: 32,
    runtimeKey: 'hit_fallback_blunt',
  },
  {
    textureId: 'hit_fallback_magic',
    category: 'effectFallbackTexture',
    size: 32,
    runtimeKey: 'hit_fallback_magic',
  },
  {
    textureId: 'buff_fallback',
...
export const BUILD_EFFECT_FALLBACK_TEXTURES_USAGE =
  'Usage: node tools/aseprite-pipeline/build-effect-fallback-textures.mjs [--ids <comma-separated-texture-ids>]';
...
function createBuildTarget(texture) {
  ...
  return {
    id: `effect_fallback_texture_${texture.textureId}`,
    ...
    effectFallbackTextureId: texture.textureId,
    size: texture.size,
  };
}
...
function runAsepriteCreate({ asepriteExe, target }) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      '--script-param',
      `effect=${target.effectFallbackTextureId}`,
      '--script-param',
      `size=${target.size}`,
      '--script',
      CREATE_SCRIPT,
  ...
  if (result.status !== 0) {
    ...
    throw new Error(`Aseprite effect fallback source generation failed for ${target.id} with status ${result.status}.${stderr}${stdout}`);
  }
}
...
function createRosterEntry(target, priority) {
  return {
    ...
    requiredTags: [],
    effectFallbackTextureId: target.effectFallbackTextureId,
  };
}
...
export function parseBuildEffectFallbackTexturesArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--ids') {
      ...
      if (!value || value.startsWith('-')) {
        throw new Error('Expected a comma-separated effect fallback texture id list after --ids.');
      }
...
export function buildEffectFallbackTextures({ ids = null, dependencies = {} } = {}) {
  ...
  const textureById = new Map(EFFECT_FALLBACK_TEXTURES.map((texture) => [texture.textureId, texture]));
  const selectedTextures = ids?.length
    ? [...new Set(ids)].sort((a, b) => a.localeCompare(b)).map((id) => {
      const texture = textureById.get(id);
      if (!texture) {
        throw new Error(`Unknown effect fallback texture id: ${id}`);
      }
      return texture;
    })
    : EFFECT_FALLBACK_TEXTURES;
  ...
  for (const texture of selectedTextures) {
    ...
    normalizeFile(
      ...
      target.effectFallbackTextureId,
...
if (isCliEntrypoint()) {
  ...
  try {
    parsedArgs = parseBuildEffectFallbackTexturesArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(BUILD_EFFECT_FALLBACK_TEXTURES_USAGE);
    process.exit(1);
  }

  try {
    console.log(JSON.stringify(buildEffectFallbackTextures(parsedArgs), null, 2));
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
