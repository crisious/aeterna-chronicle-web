...
## TODO (CHRONO-ATB chapter VIII 후보)

- REST `/combat/triple_tech` 라우트 + 클라 networkManager.combatTripleTech
- WebSocket combat:triple_tech 핸들러 (3-actor 권한 검증)
- 클라 BattleScene tripleTechCandidates 수신 + 'T' 단축키 발동
- BattleScene Triple Tech 버튼 (협공 버튼 옆)
- Triple Tech 발동 시각 효과 (Dual Tech 보다 강렬 — 3중첩 + 카메라 shake 강)
- Triple Tech 사용 통계 (snapshot에 tripleTechHitsTaken 추가)
