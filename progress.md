...
- BattleScene UI 에 era 라벨 + ATB tier 시각 표시 (라이브 검증 필요)
- 크로노 트리거 Dual Tech (2인 협공 기술) 데이터 + 트리거 로직
- 시대별 monster 차별화 (zoneSeeds + monster data 확장)
- BattleScene `init.eraId` 가 localStorage 복원으로 자동 채워지도록 ergonomics 개선 (현재는 WorldScene 경유 시에만 흐름)
